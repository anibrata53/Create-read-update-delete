...
func TestGetAllPerson(t *testing.T) {
	...
	handler := http.HandlerFunc(GetAllPerson)
...
func TestGetPersonByID(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "/get/12", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUpdatePersonByID(t *testing.T) {
	...
	req, err := http.NewRequest("PUT", "Localhost:8090/update/{id}", bytes.NewBuffer(jsonStr))
	if err != nil {
		t.Fatal(err)
	}
	req.Header.Set("Content-Type", "application/json")
	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(UpdatePersonByID)
	handler.ServeHTTP(rr, req)
	if status := rr.Code; status != http.StatusOK {
		t.Errorf("handler returned wrong status code: got %v want %v",
			status, http.StatusOK)
...
func TestDeletPersonByID(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(DeletPersonByID)
	handler.ServeHTTP(rr, req)
	checkResponseCode(t, http.StatusOK, rr.Code)
